...
class CRUD_Trichtin(CRUDBase[trichtin, trichtinCreate, trichtinUpdate]):
    def get_all_by_uid(self, uid: str, db: Session):
        result = (
            db.query(
                trichtin.uid.label("uid"),
                trichtin.ghichu_noidung.label("ghichu_noidung"),
                trichtin.updated_at.label("updated_at"),
                trichtin.uid_vaiao.label("uid_vaiao"),
                trichtin.nhanxet.label("nhanxet"),
                trichtin.xuly.label("xuly"),
                trichtin.created_at.label("created_at"),
                trichtin.created_at.label("created_at"),
    ...
    def get_trichtin_by_uid_vaiao(self, uid_vaiao: str, db: Session):
        result = (
            db.query(
                uid.name.label("hoinhom_name"),
                trichtin.ghichu_noidung.label("ghichu_noidung"),
                trichtin.updated_at.label("updated_at"),
                trichtin.uid_vaiao.label("uid_vaiao"),
                trichtin.nhanxet.label("nhanxet"),
            ...
            .filter(trichtin.uid_vaiao == uid_vaiao)
            .join(User, User.id == trichtin.user_id)
            .join(uid, uid.uid == trichtin.uid)
            .all()
        )
        formatted_result = [
            {
