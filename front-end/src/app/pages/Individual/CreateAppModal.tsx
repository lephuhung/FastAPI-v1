...
import {individual, unit, task} from './individual'
...
const ValidateIndividual = Yup.object().shape({
  ...
  id_number: Yup.string()
    .length(12)
    .matches(/^\d+$/, 'Số national_id có độ dài 12 chữ số'),
  phone_number: Yup.string()
    .length(10)
    .matches(/^\d+$/, 'Số citizen_id có độ dài 10 chữ số'),
      ...
      <div className='modal-body py-lg-10 px-lg-10'>
        <Formik
          ...
          onSubmit={(data: any) => {
            ...
            data.is_male ? (data.is_male = true) : (data.is_male = false)
            axios
              .post(`${URL}/individuals`, data)
              .then((res) => {
                if (res.status === 200) {
                  handleClose()
                ...
                <div style={{marginLeft: '30px'}}>
                  <div style={{display: 'flex', flexDirection: 'row'}}>
                    ...
                    <div style={{marginLeft: '30px', paddingTop: '40px'}}>
                      <MyCheckbox name='is_kol' setKOL ={setKOLL} is_kol= {KOLL}>KOLS</MyCheckbox>
                    </div>
                  </div>
                  <div style={{display: 'flex', flexDirection: 'row'}}>
                    <div className='mb-5'>
