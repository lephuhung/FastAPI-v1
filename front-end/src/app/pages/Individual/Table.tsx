...
import {ModalViewIndividual} from './ModalViewIndividual'
...
const Table: React.FC<Props> = ({className}) => {
  ...
  const [showModalIndividualupdate, setshowModalIndividualupdate] = useState<boolean>(false)
  const [individualitem, setindividualItem] = useState<individualResponse>()
  const [individualitemupdate, setindividualItemUpdate] = useState<individualResponse>(individualexample)
  const {isLoading, data, error} = useQuery({
    queryKey: 'individual',
    queryFn: async () => {
      const response = await axios.get(`${URL}/individuals`)
      const {data} = response
      return data
    },
  })
    ...
    <div className={`card ${className}`}>
      ...
      <div className='card-header border-0 pt-5'>
        <h3 className='card-title align-items-start flex-column'>
          <span className='card-label fw-bold fs-3 mb-1'>
            DANH SÁCH ĐỐI TƯỢNG/KOLS TRÊN ĐỊA BÀN{' '}
          </span>
      ...
      <div className='card-body py-3'>
        ...
        <div className='table-responsive'>
          ...
          <table className='table align-middle table-striped gs-0 gy-4'>
            ...
            <thead>
              <tr className='fw-bold fs-6 text-gray-800 border-bottom border-gray-200'>
                ...
                <th className='min-w-100px text-center'>LOẠI KOLS</th>
                ...
                <th className='min-w-50px text-center'>KOLs</th>
                  ...
                  >
                      ...
                      <div className='d-flex align-items-center'>
                        ...
                        <div className='d-flex justify-content-start flex-column'>
                          ...
                          <span className='text-muted fw-semibold text-muted d-block fs-7'>
                            { el.id_number ?`CCCD/CMND: ${el.id_number}`:'Không có thông tin CCCD/CMND'}
                          </span>
                        </div>
                      </div>
                    </td>
                    ...
                    <a
                      ...
                      <a
                        ...
                        className='btn btn-icon btn-bg-light btn-secondary btn-active-color-primary btn-sm me-1'
                        onClick={() => {
                          setindividualItemUpdate(el)
                          setshowModalIndividualupdate(true)
                        }}
                      >
          ...
          />
          <UpdateModal
            show={showModalIndividualupdate}
            handleClose={() => setshowModalIndividualupdate(false)}
            title='CẬP NHẬT ĐỐI TƯỢNG'
            individual={individualitemupdate}
          />
          <CreateAppModal
            show={showModal}
