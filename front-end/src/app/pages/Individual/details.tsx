...
import { type } from './individual'
type Props = {
  id: any
}
const URL = process.env.REACT_APP_API_URL

const DetailsIndividual: FC<Props> = ({id}) => {
  ...
  const result = useQueries([
    {
      queryKey: ['posts'],
      queryFn: () =>
        axios.get(`${URL}/individual/view/${id}`).then((res) => {
          return res.data
        }),
    },
    {
      queryKey: ['users'],
      queryFn: () =>
        axios.get(`${URL}/individual/details/${id}`).then((res) => {
          return res.data
        }),
    },
  ])
            ...
            <div className='card-body pt-9 pb-0'>
              <div className='d-flex flex-wrap flex-sm-nowrap mb-3'>
                <div className='me-7 mb-4'>
                  <div className='symbol symbol-100px symbol-lg-160px symbol-fixed position-relative'>
                    {result[0].data.image_url ? (
                      <img src={result[0].data?.image_url} className='' alt='' />
                    ) : (
                      <Avatar name={result[0].data.full_name} size='100' />
                    )}
                ...
                <div className='flex-grow-1'>
                  <div className='d-flex justify-content-between align-items-start flex-wrap mb-2'>
                    <div className='d-flex flex-column'>
                      <div className='d-flex align-items-center mb-2'>
                        ...
                        >
                          {result[0].data && result[0].data.full_name.toUpperCase()}
                        </a>
                        <a href='#'>
                          <KTSVG
                      ...
                      <div className='d-flex flex-wrap fw-bold fs-6 mb-4 pe-2'>
                        ...
                        >
                          ...
                          />
                          {result[0].data && result[0].data.is_kol ? 'is_kol' : 'KHÔNG'}
                        </a>
                        <a
                          href='#'
                          className='d-flex align-items-center text-gray-400 text-hover-primary me-5 mb-2'
                        >
                          <KTSVG
                            ...
                            className='svg-icon-4 me-1'
                          />
                          {result[0].data && result[0].data.hometown}
                        </a>
                        <a
                          href='#'
                        ...
                        >
                          <KTSVG
                            ...
                            className='svg-icon-4 me-1'
                          />
                          {result[0].data && `LOẠI is_kol: ${result[0].data.phone_number}`}
                        </a>
                  ...
                  <span>
                    {result[0].data ? result[0].data.additional_info : 'Chưa có thông tin'}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className='table-responsive'>
                ...
                <table className='table align-middle table-striped gs-0 gy-4'>
                  ...
                  <span className='badge badge-light-primary fs-7 fw-semibold'>{el.uid_name}</span>
                          ...
                          <td>
                            <span className='badge badge-light-success fs-7 fw-semibold'>
                              {type[el.account_type_id].name.toUpperCase()}
                            </span>
                          </td>
                        </tr>
                      ))}
