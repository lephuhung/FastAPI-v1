...
import { ITinhchatResponse } from './Tinhchat'
...
const Table: React.FC<Props> = ({ className }) => {
  ...
  const { isLoading, data, error } = useQuery({
    queryKey: ['tinhchat', page.get('tinhchat')],
    queryFn: async () => {
      const respone = await instance.get(`${URL}/tinhchat/getAll`)
      const { data } = respone
      return data
    },
  })
      ...
      <div className='card-body py-3'>
        ...
        <div className='table-responsive'>
          ...
          <table className='table align-middle gs-0 gy-4'>
            ...
            <tbody>
              {data &&
                data.map((el: ITinhchatResponse, index: number) => (
                  <tr key={index}>
                    <td>
