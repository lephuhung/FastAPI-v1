...
const Table: React.FC<Props> = ({className}) => {
  const {isLoading, data, error} = useQuery({
    queryKey: ['phanloai'],
    queryFn: async () => {
      const respone = await axios.get(`${URL}/phanloai`)
      const {data} = respone
      return data.DATA
    },
  })
    ...
    <div className={`card ${className}`}>
      ...
      <div className='card-header border-0 pt-5'>
        <h3 className='card-title align-items-start flex-column'>
          <span className='card-label fw-bold fs-3 mb-1'> PHÂN LOẠI SẮC THÁI NỘI DUNG</span>
          ...
          <table className='table align-middle gs-0 gy-4'>
            ...
            <thead>
              <tr className='fw-bold text-muted bg-light'>
                ...
                <th className='min-w-150px'>NộI DUNG</th>
                <th className='min-w-100px'>THỜI GIAN</th>
              </tr>
            </thead>
            <tbody>
              {Array.isArray(data) &&
                data.map((el: Phanloai, index: number) => (
                  <tr key={index}>
