...
export function getConfig() {
  ...
  if (
    ...
    (window as any).appConfig.engineName
  ) {
...
export function getResultTitle(result:any) {
...
export function stripUnnecessaryResultFields(resultFields:any) {
...
export function buildFacetConfigFromConfig() {
  ...
  const facets = (config.facets || []).reduce((acc:any, n:number) => {
    acc = acc || {};
    acc[n] = {
      type: "value",
      size: 100
...
export function buildSortOptionsFromConfig() {
  ...
  return [
    ...
    ...(config.sortFields || []).reduce((acc:any, sortField:any) => {
      acc.push({
