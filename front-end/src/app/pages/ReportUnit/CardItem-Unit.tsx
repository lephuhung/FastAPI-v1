...
type Props = {
  ...
  badgeColor: string
  NVCB: string
  task_hoinhom?: Array<task>
  title: string
  task_individual?: Array<task>
  //   date: string
  budget: string
  item: any
}
...
const CardItem: FC<Props> = ({
  ...
  return (
    <Link
      to={`/thongke-unit/details/${item.id}`}
      ...
      <div className='card-body p-5'>
          ...
          <div className='border border-gray-300 border-dashed rounded min-w-100px py-3 px-4 me-6 mb-3'>
            <div className='d-flex align-items-center'>
              <KTSVG
                ...
                className='svg-icon-3 svg-icon-success me-2'
              />
              <div className='fs-2 fw-bolder'>{is_kol}</div>
            </div>

            <div className='fw-bold fs-6 text-gray-400'>is_kol</div>
          </div>
          <div className='border border-gray-300 border-dashed rounded min-w-100px py-3 px-4 me-6 mb-3'>
            <div className='d-flex align-items-center'>
              <KTSVG
        ...
        <div className='d-flex flex-wrap mb-1'>
          {item.individual.task &&
            item.individual.task.map((item: task, index: number) => {
              return (
                <div
                ...
                >
                  ...
                  <div className='fw-bold fs-6 text-gray-400'>{item.task_name}</div>
                </div>
              )
            })}
        </div>
        ...
        <div className='d-flex flex-wrap mb-1'>
          {item.hoinhom.task &&
            item.hoinhom.task.map((item: task, index: number) => {
              return (
